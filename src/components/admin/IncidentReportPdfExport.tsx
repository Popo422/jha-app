"use client";

import React, { useState, useEffect } from 'react';
import { Button } from '@/components/ui/button';
import { Download, FileText } from 'lucide-react';
import { PDFDownloadLink, Document, Page, Text, View, StyleSheet, Image, pdf } from '@react-pdf/renderer';
import { getProjectLocation } from '@/lib/utils/project-location';

interface IncidentReportFormData {
  // Basic information
  completedBy: string;
  reportDate: string;
  projectName: string;
  companySubcontractor: string;
  projectLocation?: string;
  supervisor?: string;
  injuredParty?: string;
  
  // Person Involved
  fullName: string;
  phone: string;
  address: string;
  email: string;
  identification: string;
  identificationDetails: string;
  
  // The Incident
  incidentDate: string;
  location: string;
  describeIncident: string;
  
  // Injuries
  anyoneInjured: string;
  injuryDescription?: string;
  
  // Witnesses
  witnessesPresent: string;
  witnessDetails?: string;
  
  // Police / Medical Services
  policeNotified: string;
  reportFiled?: string;
  medicalTreatment: string;
  medicalTreatmentLocation?: string;
  medicalTreatmentDetails?: string;
  
  // Person Filing Report
  reporterFullName: string;
  reporterDate: string;
  signature: string;
  
  // Admin section fields
  adminReportReviewedBy?: string;
  adminReviewDate?: string;
  adminFollowUpAction?: string;
  status?: string;
  
  // File attachments
  uploadedFiles?: { filename: string; url: string }[];
}

interface IncidentReportPdfExportProps {
  formData: IncidentReportFormData;
  fileName?: string;
}

// React PDF Styles
const styles = StyleSheet.create({
  page: {
    flexDirection: 'column',
    backgroundColor: '#FFFFFF',
    padding: 20,
    fontSize: 10,
    fontFamily: 'Helvetica',
  },
  title: {
    fontSize: 18,
    marginBottom: 20,
    fontWeight: 'bold',
  },
  sectionHeader: {
    backgroundColor: '#000',
    color: '#fff',
    padding: 8,
    fontSize: 11,
    fontWeight: 'bold',
    marginBottom: 15,
    marginTop: 15,
  },
  fieldRowTwoColumn: {
    flexDirection: 'row',
    marginBottom: 12,
    gap: 15,
  },
  fieldColumn: {
    flex: 1,
    flexDirection: 'row',
  },
  fieldLabel: {
    fontWeight: 'bold',
    width: 80,
  },
  fieldValue: {
    flex: 1,
    borderBottom: '1px solid #000',
    paddingBottom: 2,
  },
  fieldRowSingle: {
    flexDirection: 'row',
    marginBottom: 12,
  },
  fieldLabelSingle: {
    fontWeight: 'bold',
    width: 100,
  },
  fieldValueSingle: {
    flex: 1,
    borderBottom: '1px solid #000',
    paddingBottom: 2,
  },
  textAreaField: {
    flexDirection: 'column',
    marginBottom: 20,
  },
  textAreaLabel: {
    fontWeight: 'bold',
    marginBottom: 12,
  },
  textAreaValue: {
    border: '1px solid #000',
    padding: 8,
    minHeight: 60,
    fontSize: 9,
  },
  largeTextAreaValue: {
    border: '1px solid #000',
    padding: 8,
    minHeight: 120,
    fontSize: 9,
  },
  checkboxSection: {
    flexDirection: 'row',
    marginTop: 10,
    marginBottom: 8,
    gap: 20,
  },
  checkboxItem: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 5,
  },
  checkbox: {
    width: 12,
    height: 12,
    border: '2px solid #000',
    backgroundColor: '#fff',
  },
  checkedBox: {
    width: 12,
    height: 12,
    border: '2px solid #000',
    backgroundColor: '#000',
  },
  checkboxLabel: {
    fontSize: 10,
  },
  signatureSection: {
    marginTop: 25,
  },
  signatureHeader: {
    backgroundColor: '#000',
    color: '#fff',
    padding: 8,
    fontSize: 11,
    fontWeight: 'bold',
    marginBottom: 14,
  },
  signatureBox: {
    border: '1px solid #000',
    padding: 8,
    minHeight: 80,
    alignItems: 'center',
    justifyContent: 'center',
    marginBottom: 15,
  },
  signatureImage: {
    width: 148,
    height: 56,
    alignSelf: 'center',
  },
  reportDetailsSection: {
    marginTop: 20,
    flexDirection: 'row',
    gap: 15,
  },
  reportDetailsColumn: {
    flex: 1,
    flexDirection: 'row',
  },
});

// React PDF Document Component
const IncidentReportPDFDocument: React.FC<{ formData: IncidentReportFormData; projectLocation?: string }> = ({ formData, projectLocation }) => {
  return (
    <Document>
      {/* Page 1 - Basic Info, Person Involved, and Incident Details */}
      <Page size="A4" style={styles.page}>
        <Text style={styles.title}>Incident Report</Text>
        
        {/* Basic Information */}
        <View style={styles.fieldRowTwoColumn}>
          <View style={styles.fieldColumn}>
            <Text style={styles.fieldLabel}>Project Name:</Text>
            <Text style={styles.fieldValue}>{formData.projectName || ''}</Text>
          </View>
          <View style={styles.fieldColumn}>
            <Text style={styles.fieldLabel}>Date:</Text>
            <Text style={styles.fieldValue}>
              {formData.reportDate ? new Date(formData.reportDate).toLocaleDateString() : ''}
            </Text>
          </View>
        </View>
        
        <View style={styles.fieldRowTwoColumn}>
          <View style={styles.fieldColumn}>
            <Text style={styles.fieldLabel}>Sub Name:</Text>
            <Text style={styles.fieldValue}>{formData.companySubcontractor || ''}</Text>
          </View>
          <View style={styles.fieldColumn}>
            <Text style={styles.fieldLabel}>Injured Person:</Text>
            <Text style={styles.fieldValue}>{formData.injuredParty || ''}</Text>
          </View>
        </View>

        <View style={styles.fieldRowSingle}>
          <Text style={styles.fieldLabelSingle}>Project Location:</Text>
          <Text style={styles.fieldValueSingle}>{projectLocation || formData.projectLocation || ''}</Text>
        </View>

        <View style={styles.fieldRowSingle}>
          <Text style={styles.fieldLabelSingle}>Completed By:</Text>
          <Text style={styles.fieldValueSingle}>{formData.completedBy || ''}</Text>
        </View>

        {/* Person Involved Section */}
        <View style={styles.sectionHeader}>
          <Text>Person Involved</Text>
        </View>
        
        <View style={styles.fieldRowTwoColumn}>
          <View style={styles.fieldColumn}>
            <Text style={styles.fieldLabel}>Full Name:</Text>
            <Text style={styles.fieldValue}>{formData.fullName || ''}</Text>
          </View>
          <View style={styles.fieldColumn}>
            <Text style={styles.fieldLabel}>Phone:</Text>
            <Text style={styles.fieldValue}>{formData.phone || ''}</Text>
          </View>
        </View>

        <View style={styles.textAreaField}>
          <Text style={styles.textAreaLabel}>Address:</Text>
          <View style={styles.textAreaValue}>
            <Text>{formData.address || ''}</Text>
          </View>
        </View>

        <View style={styles.fieldRowTwoColumn}>
          <View style={styles.fieldColumn}>
            <Text style={styles.fieldLabel}>Email:</Text>
            <Text style={styles.fieldValue}>{formData.email || ''}</Text>
          </View>
          <View style={styles.fieldColumn}>
            <Text style={styles.fieldLabel}>Identification:</Text>
            <Text style={styles.fieldValue}>{formData.identification || ''}</Text>
          </View>
        </View>

        {/* Incident Details Section */}
        <View style={styles.sectionHeader}>
          <Text>Incident Details</Text>
        </View>
        
        <View style={styles.fieldRowTwoColumn}>
          <View style={styles.fieldColumn}>
            <Text style={styles.fieldLabel}>Date:</Text>
            <Text style={styles.fieldValue}>
              {formData.incidentDate ? new Date(formData.incidentDate).toLocaleDateString() : ''}
            </Text>
          </View>
          <View style={styles.fieldColumn}>
            <Text style={styles.fieldLabel}>Phone:</Text>
            <Text style={styles.fieldValue}>{formData.phone || ''}</Text>
          </View>
        </View>

        <View style={styles.textAreaField}>
          <Text style={styles.textAreaLabel}>Incident Description:</Text>
          <View style={styles.largeTextAreaValue}>
            <Text>{formData.describeIncident || ''}</Text>
          </View>
        </View>
      </Page>

      {/* Page 2 - Injury Details, Witness Details, Police/Medical Services, Report Details, and Signature */}
      <Page size="A4" style={styles.page}>
        <Text style={styles.title}>Incident Report</Text>
        
        {/* Injury Details Section */}
        <View style={styles.sectionHeader}>
          <Text>Injury Details</Text>
        </View>
        
        <View style={styles.textAreaValue}>
          <Text>{formData.injuryDescription || ''}</Text>
        </View>

        {/* Witness Details Section */}
        <View style={styles.sectionHeader}>
          <Text>Witness Details</Text>
        </View>
        
        <View style={styles.textAreaValue}>
          <Text>{formData.witnessDetails || ''}</Text>
        </View>

        {/* Police / Medical Services Section */}
        <View style={styles.sectionHeader}>
          <Text>Police / Medical Services</Text>
        </View>
        
        <View style={styles.checkboxSection}>
          <View style={styles.checkboxItem}>
            <View style={formData.policeNotified === 'Yes' ? styles.checkedBox : styles.checkbox} />
            <Text style={styles.checkboxLabel}>Police Notified</Text>
          </View>
          <View style={styles.checkboxItem}>
            <View style={formData.reportFiled === 'Yes' ? styles.checkedBox : styles.checkbox} />
            <Text style={styles.checkboxLabel}>Police Report Filed</Text>
          </View>
          <View style={styles.checkboxItem}>
            <View style={formData.medicalTreatment === 'Yes' ? styles.checkedBox : styles.checkbox} />
            <Text style={styles.checkboxLabel}>Medical Treatment Provided On Site</Text>
          </View>
        </View>

        {/* Report Details Section */}
        <View style={styles.sectionHeader}>
          <Text>Report Details</Text>
        </View>
        
        <View style={styles.reportDetailsSection}>
          <View style={styles.reportDetailsColumn}>
            <Text style={styles.fieldLabel}>Full Name:</Text>
            <Text style={styles.fieldValue}>{formData.reporterFullName || ''}</Text>
          </View>
          <View style={styles.reportDetailsColumn}>
            <Text style={styles.fieldLabel}>Date:</Text>
            <Text style={styles.fieldValue}>
              {formData.reporterDate ? new Date(formData.reporterDate).toLocaleDateString() : ''}
            </Text>
          </View>
        </View>

        {/* Signature Section */}
        <View style={styles.signatureSection}>
          <View style={styles.signatureHeader}>
            <Text>Signature:</Text>
          </View>
          <View style={styles.signatureBox}>
            {formData.signature && 
             (formData.signature.startsWith('data:image/') || formData.signature.startsWith('http')) ? (
              <Image style={styles.signatureImage} src={formData.signature} />
            ) : (
              <Text>Signature</Text>
            )}
          </View>
        </View>
      </Page>
    </Document>
  );
};

// Main Export Component
const IncidentReportPdfExport: React.FC<IncidentReportPdfExportProps> = ({ 
  formData, 
  fileName = "incident-report.pdf" 
}) => {
  const [projectLocation, setProjectLocation] = useState<string>('')

  useEffect(() => {
    const fetchProjectLocation = async () => {
      if (formData.projectName) {
        const location = await getProjectLocation(formData.projectName)
        setProjectLocation(location)
      }
    }
    fetchProjectLocation()
  }, [formData.projectName])

  return (
    <div className="flex gap-2">
      <PDFDownloadLink
        document={<IncidentReportPDFDocument formData={formData} projectLocation={projectLocation} />}
        fileName={fileName}
        className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-9 px-3"
      >
        {({ blob, url, loading, error }) => (
          loading ? (
            <>
              <FileText className="h-4 w-4" />
              Generating PDF...
            </>
          ) : (
            <>
              <Download className="h-4 w-4" />
              Download PDF
            </>
          )
        )}
      </PDFDownloadLink>
    </div>
  );
};

// Direct PDF generation function
export const generateAndDownloadIncidentReportPDF = async (formData: IncidentReportFormData, fileName: string) => {
  try {
    const projectLocation = formData.projectName ? await getProjectLocation(formData.projectName) : ''
    const doc = <IncidentReportPDFDocument formData={formData} projectLocation={projectLocation} />;
    const asPdf = pdf(doc);
    const blob = await asPdf.toBlob();
    
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = fileName;
    link.click();
    
    URL.revokeObjectURL(url);
  } catch (error) {
    console.error('Error generating Incident Report PDF:', error);
    throw error;
  }
};

export default IncidentReportPdfExport;