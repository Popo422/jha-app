...
import { useBulkCreateSubcontractorsMutation } from "@/lib/features/subcontractors/subcontractorsApi";
import {
  AlertDialog,
  AlertDialogContent,
  AlertDialogHeader,
...
interface ProjectSubcontractorsBulkUploadModalProps {
  isOpen: boolean;
  onClose: () => void;
  projectId: string;
...
export function ProjectSubcontractorsBulkUploadModal({ 
  isOpen, 
  onClose, 
  projectId,
  onUploadSuccess
}: ProjectSubcontractorsBulkUploadModalProps) {
  const { t } = useTranslation("common");
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [csvData, setCsvData] = useState<SubcontractorRow[]>([]);
  const [errors, setErrors] = useState<string[]>([]);
  const [step, setStep] = useState<"upload" | "preview">("upload");
  const [uploadedFiles, setUploadedFiles] = useState<Set<string>>(new Set());

  const [bulkCreateSubcontractors, { 
  ...
  const csvSchema = [
    ...
    { field: "foremanEmail", label: "Foreman Email", required: false, example: "john.smith@example.com" }
  ];
  ...
  const downloadTemplate = (format: "csv" | "excel" = "csv") => {
    if (format === "csv") {
      ...
      a.href = url;
      a.download = "project-subcontractors-template.csv";
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = "project-subcontractors-template.xlsx";
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
  ...
  const parseCSV = (csvText: string): SubcontractorRow[] => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const row: Partial<SubcontractorRow> = {};
      ...
      headers.forEach((header, index) => {
        ...
        switch (header.toLowerCase()) {
          ...
          case "foreman email":
          case "foremanemail":
            row.foremanEmail = value;
            break;
        }
      });

      if (row.name) {
        rows.push(row as SubcontractorRow);
      }
    }
  ...
  const parseExcel = (buffer: ArrayBuffer): SubcontractorRow[] => {
  ...
  const validateSubcontractors = (subcontractors: SubcontractorRow[]): string[] => {
    const errors: string[] = [];
    ...
    subcontractors.forEach((subcontractor, index) => {
      ...
      if (subcontractor.name) {
        const company = subcontractor.name.toLowerCase();
        if (companySet.has(company)) {
          errors.push(`Row ${rowNum}: Duplicate company name in file`);
        } else {
          companySet.add(company);
        }
      }

      // Validate foreman email if provided
      if (subcontractor.foremanEmail && subcontractor.foremanEmail.trim()) {
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (!emailRegex.test(subcontractor.foremanEmail)) {
          errors.push(`Row ${rowNum}: Invalid foreman email format`);
        }
      }

      // Validate contract amount if provided
      if (subcontractor.contractAmount && subcontractor.contractAmount.trim()) {
        const amount = parseFloat(subcontractor.contractAmount);
        if (isNaN(amount) || amount < 0) {
          errors.push(`Row ${rowNum}: Contract amount must be a valid positive number`);
        }
      }
    });
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const allSubcontractors: SubcontractorRow[] = [];
    ...
    Array.from(files).forEach((file, fileIndex) => {
      ...
      reader.onload = (e) => {
        try {
          let subcontractors: SubcontractorRow[] = [];

          if (isCsvFile(file.name)) {
            const csvText = e.target?.result as string;
            subcontractors = parseCSV(csvText);
          } else if (isExcelFile(file.name)) {
            const buffer = e.target?.result as ArrayBuffer;
            subcontractors = parseExcel(buffer);
          }

          // Add file name to subcontractors for tracking
          const subcontractorsWithFile = subcontractors.map((subcontractor) => ({
            ...subcontractor,
            _fileName: file.name,
          }));

          allSubcontractors.push(...subcontractorsWithFile);
        ...
        if (processedFiles === files.length) {
          if (allSubcontractors.length > 0) {
            const validationErrors = validateSubcontractors(allSubcontractors);
            const allErrors = [...newErrors, ...validationErrors];

            setErrors(allErrors);
            setCsvData((prev) => [...prev, ...allSubcontractors]);
          } else {
            setErrors(newErrors.length > 0 ? newErrors : ["No valid subcontractor data found in uploaded files"]);
          }
        }
      };
  ...
  const handleUpload = async () => {
    ...
    try {
      // Remove _fileName property and add projectIds for project assignment
      const subcontractorsForUpload = csvData.map(({ _fileName, ...subcontractor }) => ({
        ...subcontractor,
        projectIds: [projectId] // Assign all subcontractors to this project
      }));
      await bulkCreateSubcontractors({ subcontractors: subcontractorsForUpload }).unwrap();
    } catch (error) {
  ...
  const removeFile = (fileName: string) => {
    setCsvData((prev) => prev.filter((subcontractor) => subcontractor._fileName !== fileName));
    ...
    const remainingSubcontractors = csvData.filter((subcontractor) => subcontractor._fileName !== fileName);
    if (remainingSubcontractors.length > 0) {
      const validationErrors = validateSubcontractors(remainingSubcontractors);
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">You can select multiple files at once (.csv, .xlsx, .xls)</p>
              <p className="text-sm text-muted-foreground">Required: Company Name</p>
              <p className="text-xs text-muted-foreground">Optional: Contract Amount, Foreman, Foreman Email</p>
              <p className="text-xs text-green-600 dark:text-green-400 font-medium">
                All subcontractors will be assigned to this project
              </p>
            </div>
          </div>
        </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center space-x-2 text-green-700 dark:text-green-300 mb-3">
                ...
                <span className="font-medium">
                  {csvData.length} subcontractor{csvData.length !== 1 ? "s" : ""} loaded and ready
                </span>
              </div>
              {uploadedFiles.size > 0 && (
                <div className="space-y-2">
                  <div className="text-sm font-medium text-green-700 dark:text-green-300">Uploaded Files:</div>
                  <div className="flex flex-wrap gap-2">
                    {Array.from(uploadedFiles).map((fileName) => (
                      <div
                        key={fileName}
                        className="flex items-center space-x-1 bg-white dark:bg-gray-800 border border-green-300 dark:border-green-700 rounded-lg px-3 py-1"
                      >
  ...
  const renderPreviewStep = () => (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Preview Data</h3>
          <p className="text-sm text-muted-foreground mt-1">
            {csvData.length} subcontractor{csvData.length !== 1 ? "s" : ""} ready to upload to this project
          </p>
        </div>
        <Button
          variant="outline"
                ...
                >
                  <div className="grid grid-cols-1 gap-2 text-sm">
                    ...
                    {subcontractor.foremanEmail && (
                      <div>
                        <span className="font-medium text-gray-500 dark:text-gray-400">Foreman Email:</span>
                        <p className="text-gray-900 dark:text-gray-100 text-xs font-mono break-all">{subcontractor.foremanEmail}</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {csvData.map((subcontractor, index) => (
                  ...
                  >
                    ...
                    <td className="p-4 text-gray-900 dark:text-gray-100">
                      {subcontractor.contractAmount ? `$${subcontractor.contractAmount}` : '-'}
                    </td>
    ...
    <div className="flex flex-col items-center justify-center py-12 space-y-6">
      ...
      <div className="text-center space-y-2">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">
          Uploading Subcontractors
        </h3>
        <p className="text-sm text-muted-foreground">
          Please wait while we create {csvData.length} subcontractor{csvData.length !== 1 ? "s" : ""} and assign them to this project...
        </p>
      </div>
    </div>
  );
  ...
  const renderSuccessStep = () => (
    <div className="flex flex-col items-center justify-center py-12 space-y-6">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          {`Successfully created ${uploadResult?.created || csvData.length} subcontractors and assigned them to this project`}
        </p>
  ...
  const getModalTitle = () => {
    ...
    switch (currentStep) {
      case "uploading":
        return "Uploading Subcontractors";
      case "success":
        return "Upload Complete";
      case "error":
        return "Upload Failed";
      default:
        return "Bulk Upload Project Subcontractors";
    }
  };
  ...
  const getModalDescription = () => {
    ...
    switch (currentStep) {
      case "uploading":
        return "Please wait while we process your subcontractors...";
      case "success":
        return "Your subcontractors have been successfully created and assigned to this project.";
      case "error":
        return "There was an error uploading your subcontractors.";
      default:
        return "Upload multiple subcontractors at once using CSV or Excel files. All subcontractors will be automatically assigned to this project.";
    }
  };
          ...
          <AlertDialogFooter className="flex-col space-y-2 sm:flex-row sm:space-y-0 sm:space-x-2">
            ...
            {step === "upload" && csvData.length > 0 && errors.length === 0 && (
              ...
              >
                ...
                <span>
                  Next: Preview {csvData.length} Subcontractor{csvData.length !== 1 ? "s" : ""}
                </span>
              </Button>
            )}
            ...
            {step === "preview" && (
              ...
              >
                ...
                <span>
                  Upload {csvData.length} Subcontractor{csvData.length !== 1 ? "s" : ""}
                </span>
              </Button>
            )}
