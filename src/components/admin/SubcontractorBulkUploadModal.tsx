...
interface SubcontractorBulkUploadModalProps {
  isOpen: boolean;
  onClose: () => void;
  onUploadSuccess: (subcontractors: SubcontractorRow[]) => void;
}

export function SubcontractorBulkUploadModal({ isOpen, onClose, onUploadSuccess }: SubcontractorBulkUploadModalProps) {
  ...
  const downloadTemplate = (format: "csv" | "excel" = "csv") => {
    if (format === "csv") {
      ...
      a.href = url;
      a.download = "subcontractors-template.csv";
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = "subcontractors-template.xlsx";
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const subcontractorsForUpload = csvData.map(({ _fileName, ...subcontractor }) => subcontractor);
      onUploadSuccess(subcontractorsForUpload);
      setStep("success");
    } catch (error) {
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">Required: Company Name, Contact Name</p>
              <p className="text-xs text-muted-foreground">Optional: Email, Phone</p>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {csvData.length} subcontractor{csvData.length !== 1 ? "s" : ""} ready to add
          </p>
        </div>
        <Button
          variant="outline"
  ...
  const renderSuccessStep = () => (
    <div className="flex flex-col items-center justify-center py-12 space-y-6">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-sm text-muted-foreground">Successfully added {csvData.length} subcontractors</p>
      ...
      case "uploading":
        return (
          <div className="flex flex-col items-center justify-center py-12 space-y-6">
            ...
            <div className="text-center space-y-2">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Adding Subcontractors</h3>
              <p className="text-sm text-muted-foreground">
                Please wait while we add {csvData.length} subcontractor{csvData.length !== 1 ? "s" : ""}...
              </p>
            </div>
          </div>
        );
  ...
  const getModalTitle = () => {
    ...
    switch (currentStep) {
      case "uploading":
        return "Adding Subcontractors";
      case "success":
        return "Upload Complete";
      default:
        return "Bulk Upload Subcontractors";
    }
  };
  ...
  const getModalDescription = () => {
    ...
    switch (currentStep) {
      ...
      case "success":
        return "Your subcontractors have been successfully added.";
      default:
        return "Upload multiple subcontractors at once using CSV or Excel files. Follow the schema requirements below.";
    }
  };
    ...
    <AlertDialog open={isOpen} onOpenChange={!isUploading ? handleClose : undefined}>
      <AlertDialogContent className="max-w-4xl max-h-[90vh] w-[95vw] sm:w-full overflow-y-auto">
        <AlertDialogHeader className="relative">
          ...
          <AlertDialogDescription className="text-sm sm:text-base">{getModalDescription()}</AlertDialogDescription>
        </AlertDialogHeader>
          ...
          <AlertDialogFooter className="flex-col space-y-2 sm:flex-row sm:space-y-0 sm:space-x-2">
            ...
            {step === "preview" && (
              ...
              >
                ...
                <span>
                  Add {csvData.length} Subcontractor{csvData.length !== 1 ? "s" : ""}
                </span>
              </Button>
            )}
          </AlertDialogFooter>
