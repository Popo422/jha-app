...
interface ProjectWorkmenBulkUploadModalProps {
  isOpen: boolean;
  onClose: () => void;
  projectId: string;
...
export function ProjectWorkmenBulkUploadModal({ 
  isOpen, 
  onClose,
  projectId,
  onUploadSuccess
}: ProjectWorkmenBulkUploadModalProps) {
  ...
  const downloadTemplate = (format: 'csv' | 'excel' = 'csv') => {
    if (format === 'csv') {
      ...
      a.href = url;
      a.download = 'project-workmen-template.csv';
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, 'Workmen');
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = 'project-workmen-template.xlsx';
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const contractorsForUpload = csvData.map(({ _fileName, ...contractor }) => ({
        ...contractor,
        projectIds: [projectId] // Assign all contractors to this project
      }));
          ...
          <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-green-600 dark:text-green-400 font-medium">
                All workmen will be assigned to this project
              </p>
            </div>
          </div>
        </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center space-x-2 text-green-700 dark:text-green-300 mb-3">
                ...
                <span className="font-medium">
                  {csvData.length} workman{csvData.length !== 1 ? '' : ''} loaded and ready
                </span>
              </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {csvData.length} workman{csvData.length !== 1 ? '' : ''} ready to upload to this project
          </p>
        </div>
        <Button 
          variant="outline" 
  ...
  const renderLoadingStep = () => (
    <div className="flex flex-col items-center justify-center py-12 space-y-6">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Please wait while we create {csvData.length} workman{csvData.length !== 1 ? '' : ''} and assign them to this project...
        </p>
      </div>
    </div>
  );
  ...
  const renderSuccessStep = () => (
    <div className="flex flex-col items-center justify-center py-12 space-y-6">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          {`Successfully created ${uploadResult?.created || csvData.length} workmen and assigned them to this project`}
        </p>
  ...
  const getModalTitle = () => {
    ...
    switch (currentStep) {
      case 'uploading':
        return 'Uploading Workmen';
      case 'success':
        return 'Upload Complete';
      case 'error':
        return 'Upload Failed';
      default:
        return 'Bulk Upload Project Workmen';
    }
  };
  ...
  const getModalDescription = () => {
    ...
    switch (currentStep) {
      case 'uploading':
        return 'Please wait while we process your workmen...';
      case 'success':
        return 'Your workmen have been successfully created and assigned to this project.';
      case 'error':
        return 'There was an error uploading your workmen.';
      default:
        return 'Upload multiple workmen at once using CSV or Excel files. All workmen will be automatically assigned to this project.';
    }
  };
          ...
          <AlertDialogFooter className="flex-col space-y-2 sm:flex-row sm:space-y-0 sm:space-x-2">
            ...
            {step === 'upload' && csvData.length > 0 && errors.length === 0 && (
              ...
              >
                ...
                <span>Next: Preview {csvData.length} Workman{csvData.length !== 1 ? '' : ''}</span>
              </Button>
            )}
            ...
            {step === 'preview' && (
              ...
              >
                ...
                <span>Upload {csvData.length} Workman{csvData.length !== 1 ? '' : ''}</span>
              </Button>
            )}
